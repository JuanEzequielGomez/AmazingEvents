...
function mostrarCartas(cartas) {
  ...
  if (cartas.length > 0) {
    ...
    <div class="card text-center">
      ...
      contenedorDatos.innerHTML = agregar;

    });
  } else {
    contenedorDatos.innerHTML = `<div class="error"><img class="error-img" src="../assets/imgs/404_2.png" alt="error 404"></div>`;
...
function superFiltro() {
  ...
  if (checkboxesEnArray.length == 0 && barraBusqueda == "") {
    filtroEnArray = eventosData;

  } 
   if (checkboxesEnArray.length > 0 && barraBusqueda !== "") {
    checkboxesEnArray.forEach((categoria) => {
      filtroEnArray.push(eventosData.filter((e) =>e.name.toLowerCase().includes(barraBusqueda.trim()) && e.category == categoria));
    });
   ...
   if (checkboxesEnArray.length > 0 && barraBusqueda == "") {
    ...
    checkboxesEnArray.forEach((categoria) => {
      filtroEnArray.push(eventosData.filter((e) => e.category == categoria));
    });

  } 
   if (checkboxesEnArray.length == 0 && barraBusqueda !== "") {
    filtroEnArray.push(eventosData.filter((e) =>e.name.toLowerCase().includes(barraBusqueda.trim())));
  }

  mostrarCartas(filtroEnArray.flat());
  console.log(filtroEnArray);
}
...
//   card.className="card"
//   card.style.width="18rem"
//   card.innerHTML=`<img src="${data.events[i].image}" class="card-img-top" alt="...">
//   <div class="card-body d-flex flex-column justify-content-between text-center">
//     <h5 class="card-title">${data.events[i].name}</h5>
//     <p class="card-text">${data.events[i].description}</p>
//     <p class="card-text">${data.events[i].place}</p>
//     <div class="d-flex justify-content-between p-4">
//       <p class="card-price">price: €${data.events[i].price}</p>
//       <a href="./details.html" class="btn btn-primary">More info</a>
//     </div>
//   </div>`
